...
wd='/home/jxlabbrain/02.wjc/02.scanpy/05.Fetus'
...
MT_gene_file = open('/home/jxlabbrain/02.wjc/05.ref/reference/MT_genes','r')
...
# 通过运行主成分分析 （PCA） 来降低数据的维数，PCA 可以揭示变异的主轴并对数据进行降噪。
...
#进行Unique操作
# 读取 CSV 文件
df = pd.read_csv('/home/jxlabbrain/02.wjc/02.scanpy/01.Fetus_E1E2E9/E1E2E9_non_Pineal_BBKNN_cellinfo.csv')

# 按照 TissueName, Organ, Sex 分组，并计算每个组合的行数
result = df.groupby(["TissueName", "Organ", "Sex","Platform1"]).size().reset_index(name="cell_number")
# 计算 n_gene 和 doublet_score 的平均值
mean_n_gene = df.groupby(["TissueName", "Organ", "Sex","Platform1"])["n_genes"].mean().reset_index(name="mean_n_gene")
mean_doublet_score = df.groupby(["TissueName", "Organ", "Sex","Platform1"])["doublet_score"].mean().reset_index(name="mean_doublet_score")
# 合并结果
result = result.merge(mean_n_gene, on=["TissueName", "Organ", "Sex","Platform1"])
result = result.merge(mean_doublet_score, on=["TissueName", "Organ", "Sex","Platform1"])
# 重命名列（如果需要）
result.columns = ["TissueName", "Organ", "Sex","Platform1", "cell_number", "mean_n_gene", "mean_doublet_score"]
# 保存结果到新的 CSV 文件
result.to_csv("unique_mnn_cellinfo.csv", index=False)

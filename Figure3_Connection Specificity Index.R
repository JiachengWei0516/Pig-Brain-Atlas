...
data_astro = readRDS('D:/Desktop/mnn_counts_astro.rds')

loom <- open_loom("D:/Desktop/out_SCENIC_Astro_mc9nr.loom")
...
sub_regulonAUC <- regulonAUC[,match(colnames(data_astro),colnames(regulonAUC))]
identical(colnames(sub_regulonAUC), colnames(data_astro))

data_astro@meta.data = cbind(data_astro@meta.data ,t(sub_regulonAUC@assays@data$AUC))

rss<-calcRSS(AUC=getAUC(sub_regulonAUC),cellAnnotation=data_astro@meta.data$celltype8)
pdf('rss_plot_Astro_state.pdf')
rssPlot <- plotRSS(rss)
rssPlot$plot
dev.off()
...
data_astro = readRDS('D:/Desktop/mnn_counts_astro.rds')
data_astro@meta.data <- cbind(data_astro@meta.data, module_activity)

data_astro <- readRDS('D:/Desktop/Astro_module_activity_M1_M12.rds')
...
FeaturePlot(
  ...
  scale_color_gradientn(
    ...
    limits = c(0, 0.25),        
    breaks = my_breaks,         
    values = scales::rescale(c(0, 0.12, 0.16, 0.25)) 
  ) +
  ggtitle("M2")
